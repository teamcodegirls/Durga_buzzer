...
.directive('googleAutocompleteSuggestion', function($document, $ionicModal, $ionicTemplateLoader, googleAutocompleteService) {
    return {
        ...
        link: function($scope, element) {
            ...
            var template = [
                '<ion-modal-view>',
                '<ion-header-bar class="item-input-inset padding">',
                '<label class="item-input-wrapper">',
                '<i class="icon ion-search placeholder-icon"></i>',
                '<input type="text" ng-model="search.query" placeholder="Search">',
                '</label>',
                '<button class="ion-autocomplete-cancel button button-clear button-dark ng-binding" ng-click="close()" translate style="color:gray;">X</button>',
                '</ion-header-bar>',
                '<ion-content>',
                '<ion-list>',
